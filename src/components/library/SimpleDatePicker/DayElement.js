...
function DayElement(props) {
	...
	return (
		<select id="date-day" onClick={props.validateDay} onChange={(e)=>{props.onChangeDay({day : Number(e.target.value)})}} value={props.selectedDay} disabled={props.disableSelect} >{options}</select>
	)
}
